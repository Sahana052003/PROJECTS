      ...
      <div class="col-md-6">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <%-- Error: wrong OTP (from server) --%>
            <c:if test="${not empty otpError}">
              <div class="alert-error-custom" id="serverOtpError">
                ${otpError}
              </div>
            </c:if>
            ...
            <form action="verifyOtp" method="post" onsubmit="return validateOtpForm()">
              ...
              <div class="mb-4">
                ...
                <input type="text"
                       id="otpField"
                       name="otp"
                       class="otp-input ${not empty otpError ? 'is-invalid' : ''}"
                       maxlength="6"
                       placeholder="------"
                       autocomplete="one-time-code">
                <%-- Client-side error span (JS ON) --%>
...
document.getElementById("otpField").addEventListener("input", function () {
...
document.getElementById("otpField").addEventListener("keypress", function (e) {
    if (!/[0-9]/.test(e.key)) {
        e.preventDefault();
    }
});
...
window.onload = function () {
    ...
    if (success) setTimeout(function () { success.style.display = "none"; }, 5000);

    var err = document.getElementById("serverOtpError");
