...
.back-link a { color:#7A1FC0; text-decoration:none; font-size:0.9rem; }
...
<div class="form-wrapper">
  <div class="container">
    <div class="row justify-content-center">
      <div class="col-md-6">
        <div class="card">
          <div class="card-header-gradient">Forget Password</div>
          <div class="card-body">


          <!--  Error message from server -->
          <c:if test="${not empty emailError}">
                       <div class="alert alert-danger text-center mb-3" id="serverError">
                         ${emailError}
                       </div>
                     </c:if>
          ...
          <form action="verifyEmail" method="get"
                       ...
                       <div class="mb-4">
                         ...
                         <%-- CLIENT error span (shows when JS is ON) --%>
                         <div class="text-danger small mt-1" id="emailClientError"></div>
                         <%-- SERVER error below input (shows when JS is OFF) --%>
                         <c:if test="${not empty emailError}">
...
<script>
// JS ON: client-side validation
function validateEmailForm() {
    var email   = document.getElementById("emailId");
    var errSpan = document.getElementById("emailClientError");
    var pattern = /^[a-zA-Z0-9._%+-]+@gmail\.com$/;

    if (!email.value.trim()) {
        email.classList.add("is-invalid");
        errSpan.innerText = "Email cannot be empty";
        return false;
    }
    if (!pattern.test(email.value.trim())) {
        email.classList.add("is-invalid");
        errSpan.innerText = "Please enter a valid @gmail.com email";
        return false;
    }
...
document.getElementById("emailId").addEventListener("keyup", function () {
    var pattern = /^[a-zA-Z0-9._%+-]+@gmail\.com$/;
    var err     = document.getElementById("emailClientError");
    if (!this.value.trim()) {
        this.classList.add("is-invalid");
        err.innerText = "Email cannot be empty";
    } else if (!pattern.test(this.value.trim())) {
        this.classList.add("is-invalid");
        err.innerText = "Please enter a valid @gmail.com email";
    } else {
...
// Auto-hide server error after 4 seconds
window.onload = function () {
    var box = document.getElementById("serverError");
