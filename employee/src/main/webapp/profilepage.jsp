...
body { font-family:'Segoe UI',sans-serif; background:#f4f5f7; min-height:100vh; }
...
.profile-dropdown {
    ...
    box-shadow:0 8px 20px rgba(0,0,0,0.15); z-index:1000;
}
.dropdown-wrapper[open] .profile-dropdown { display:block; }
.dropdown-header { padding:12px; background:#7A1FC0; color:white; border-radius:12px 12px 0 0; }
.dropdown-item-custom, .dropdown-item-logout {
    display:block; padding:10px; text-decoration:none; color:black;
}
.dropdown-item-logout { color:red; }
.dropdown-item-custom:hover, .dropdown-item-logout:hover { background:#eee; }

/* ── Page ── */
.page { max-width:680px; margin:36px auto; padding:0 20px 60px; }
...
/* ★ Profile photo - circular, shown next to name ★ */
.profile-photo {
...
.alert-success-custom {
    ...
    border-radius:10px; padding:12px 18px; margin-bottom:16px; font-weight:500;
}
</style>
</head>
<body>
...
<nav class="navbar bg-gradient-purple">
  <div class="container-fluid navbar-container">
   ...
   <!-- Avatar Dropdown — replace the existing details block -->
   <div class="nav-right">
     <details class="dropdown-wrapper">

       <%-- Navbar avatar: show photo if available, letter if not --%>
       <summary class="avatar-btn" style="overflow:hidden; padding:0;">
         <c:choose>
           <c:when test="${not empty loggedInUser.imagePath}">
             <c:url var="navImgUrl" value="/serveImage">
       ...
       <div class="profile-dropdown">
         <div class="dropdown-header">${loggedInUser.emailId}</div>
