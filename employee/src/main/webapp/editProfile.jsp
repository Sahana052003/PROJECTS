<%@ page isELIgnored="false" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>

<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Edit Profile</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
<style>
* { margin:0; padding:0; box-sizing:border-box; }
body {
    min-height: 100vh;
    background: linear-gradient(135deg, #0d0d1a, #2d0a4e, #0d0d1a);
    font-family: 'Segoe UI', sans-serif;
}
.navbar-container {
    height: 100px;
    display: flex;
    justify-content: space-between;
    align-items: center;
    width: 100%;
}
.navbar-brand { margin-left: 25px; }
.nav-right {
    display: flex;
    margin-right: 25px;
    align-items: center;
    position: relative;
}
.bg-gradient-purple {
    background: linear-gradient(90deg, #9370DB, #4B0082);
}
.avatar-btn {
    width: 48px; height: 48px;
    border-radius: 50%;
    background: linear-gradient(135deg, #B19CD9, #7A1FC0);
    color: white; font-weight: bold; font-size: 1.4rem;
    display: flex; align-items: center; justify-content: center;
    cursor: pointer; user-select: none;
    border: 2px solid rgba(255,255,255,0.6);
    transition: transform 0.2s ease;
}
.avatar-btn:hover { transform: scale(1.08); }
.profile-dropdown {
    display: none; position: absolute;
    top: 62px; right: 0;
    background: white; border-radius: 14px;
    min-width: 230px;
    box-shadow: 0 8px 28px rgba(0,0,0,0.18);
    z-index: 999; overflow: hidden;
    border: 1px solid #e0daf7;
    animation: fadeIn 0.2s ease;
}
.profile-dropdown.show { display: block; }
@keyframes fadeIn {
    from { opacity:0; transform:translateY(-8px); }
    to   { opacity:1; transform:translateY(0); }
}
.dropdown-header {
    padding: 16px 18px;
    background: linear-gradient(90deg, #9370DB, #4B0082);
    color: white;
}
.dropdown-header .dd-label { font-size:0.78rem; opacity:0.8; margin-bottom:3px; }
.dropdown-header .dd-email { font-weight:bold; font-size:0.95rem; word-break:break-all; }
.dropdown-item-custom {
    display: flex; align-items: center; gap: 10px;
    padding: 13px 18px; color: #333;
    text-decoration: none; font-size: 0.95rem;
    border-bottom: 1px solid #f0ecfa;
    transition: background 0.15s;
}
.dropdown-item-custom:hover { background: #f5f0ff; color: #5D0E99; }
.dropdown-item-logout {
    display: flex; align-items: center; gap: 10px;
    padding: 13px 18px; color: #842029;
    text-decoration: none; font-size: 0.95rem;
    transition: background 0.15s;
}
.dropdown-item-logout:hover { background: #fff5f5; }

/* Card */
.card {
    border-radius: 20px !important;
    overflow: hidden;
    box-shadow: 0 8px 32px rgba(0,0,0,0.18);
}
.card-header-gradient {
    background: linear-gradient(90deg, #9370DB, #4B0082);
    color: white; font-weight: bold;
    font-size: 1.3rem; text-align: center;
    padding: 18px;
}
.card .form-control,
.card .form-select {
    border-radius: 12px;
    border: 1.5px solid #e0daf7;
    height: 48px;
}
.card textarea.form-control { height: auto; padding-top: 10px; }
.card .form-control:focus,
.card .form-select:focus {
    border-color: #7A1FC0;
    box-shadow: 0 0 0 3px rgba(122,31,192,0.12);
}
.form-label { font-weight: 500; color: #333; }
.email-locked { color: #555 !important; cursor: not-allowed; background: #f8f5ff; }

/* Update button */
.btn-update {
    background: linear-gradient(45deg, #B19CD9, #7A1FC0);
    color: white; font-weight: bold;
    font-size: 1.1rem; padding: 12px;
    width: 100%; border: none;
    border-radius: 20px;
    box-shadow: 0 4px 10px rgba(0,0,0,0.25);
    transition: all 0.3s ease;
}
.btn-update:hover {
    background: linear-gradient(45deg, #A083D1, #5D0E99);
    transform: scale(1.02);
}

/* Validation */
.is-invalid {
    border-color: #dc3545 !important;
}
.is-invalid ~ .invalid-feedback {
display: block !important;
}

.text-danger {
    font-size: 0.9rem;
    margin-top: 4px;
}

/* ── Photo upload circle ── */
.photo-wrap {
    position: relative;
    width: 110px; height: 110px;
    margin: 0 auto 6px;
    cursor: pointer;
}
.photo-circle {
    width: 110px; height: 110px;
    border-radius: 50%;
    object-fit: cover;
    border: 3px solid #9370DB;
    display: block;
}
.camera-overlay {
    position: absolute;
    bottom: 0; left: 50%;
    transform: translateX(-50%);
    background: #22c55e;
    color: white;
    border-radius: 20px;
    padding: 5px 14px;
    font-size: 13px;
    font-weight: 500;
    display: flex;
    align-items: center;
    gap: 5px;
    white-space: nowrap;
    pointer-events: none;
}
.photo-filename {
    font-size: 12px;
    color: #7A1FC0;
    text-align: center;
    min-height: 18px;
    margin-top: 4px;
}

</style>
</head>

<body>

<!-- Navbar -->
<nav class="navbar bg-gradient-purple">
  <div class="container-fluid navbar-container">
    <a class="navbar-brand" href="#">
      <img src="data:image/jpeg;base64,"
           alt="X-Workz Logo" height="70"
           style="border-radius:50%; object-fit:cover;">
    </a>

    <!-- Avatar  -->
    <div class="nav-right">
      <div class="avatar-btn" id="profileBtn" onclick="toggleDropdown()">
        ${fn:toUpperCase(fn:substring(loggedInUser.emailId, 0, 1))}
      </div>

      <div class="profile-dropdown" id="profileDropdown">
        <div class="dropdown-header">
          <div class="dd-label">Signed in as</div>
          <div class="dd-email">${loggedInUser.emailId}</div>
        </div>
        <a href="editProfile?id=${loggedInUser.id}"
           class="dropdown-item-custom">
           Edit Profile
        </a>
        <a href="login" class="dropdown-item-logout">
           Sign Out
        </a>
      </div>
    </div>

  </div>
</nav>

<!-- Edit Profile Form -->
<div class="container mt-5 mb-5">
  <div class="row justify-content-center">
    <div class="col-md-6">
      <div class="card shadow">

        <div class="card-header card-header-gradient">
          Edit Profile
        </div>

        <div class="card-body" style="background:#fff; padding:32px;">

               <form action="updateEmployee" method="post"
               onsubmit="return validateEditForm()" enctype="multipart/form-data">

<!-- ── Profile Photo Upload ── -->
<div class="mb-4 text-center">
  <label class="form-label d-block">Profile Photo</label>

  <div class="photo-wrap" onclick="document.getElementById('file').click()"
       title="Click to change photo">

    <%-- Show existing photo OR purple letter avatar --%>
    <c:choose>
      <c:when test="${not empty employeeDTO.imagePath}">
        <c:url var="existingImg" value="/serveImage">
          <c:param name="path" value="${employeeDTO.imagePath}"/>
        </c:url>
        <img id="photoPreview" class="photo-circle"
             src="${existingImg}"
             alt="Profile photo"
             onerror="this.src='data:image/svg+xml,...'">
      </c:when>
      <c:otherwise>
        <img id="photoPreview" class="photo-circle"
             src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 110 110'%3E%3Ccircle cx='55' cy='55' r='55' fill='%23e0daf7'/%3E%3Ccircle cx='55' cy='42' r='18' fill='%239370DB'/%3E%3Cellipse cx='55' cy='90' rx='28' ry='20' fill='%239370DB'/%3E%3C/svg%3E"
             alt="Profile photo">
      </c:otherwise>
    </c:choose>

    <div class="camera-overlay">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none"
           stroke="currentColor" stroke-width="2"
           stroke-linecap="round" stroke-linejoin="round">
        <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/>
        <circle cx="12" cy="13" r="4"/>
      </svg>
      Edit
    </div>
  </div>

  <div class="photo-filename" id="photoName"></div>

  <%-- Hidden file input — triggered by clicking the circle above --%>
  <input type="file" id="file" name="file" accept="image/*"
         style="display:none" onchange="previewNewImage(event)">
</div>
            <!-- Hidden ID — MUST be submitted -->
            <!-- Hidden fields — required by controller -->
                        <input type="hidden" name="id"           value="${employeeDTO.id}">
                        <input type="hidden" name="mobileNumber" value="${employeeDTO.mobileNumber}">
                        <input type="hidden" name="gender"       value="${employeeDTO.gender}">

                        <!-- NAME -->
                        <div class="mb-3">
                          <label class="form-label">User Name</label>
                          <input type="text"
                                 class="form-control ${not empty userNameError ? 'is-invalid' : ''}"
                                 id="userName" name="userName"
                                 value="${employeeDTO.userName}">
                          <span class="text-danger" id="nameError">
                            <c:if test="${not empty userNameError}">${userNameError}</c:if>
                          </span>
                        </div>

                        <!-- EMAIL (readonly — value sent via hidden input if needed) -->
                        <div class="mb-3">
                          <label class="form-label">Email ID</label>
                          <input type="email" class="form-control email-locked"
                                 name="emailId"
                                 value="${employeeDTO.emailId}"
                                 readonly>
                        </div>

                        <!-- AGE -->
                        <div class="mb-3">
                          <label class="form-label">Age</label>
                          <input type="number"
                                 class="form-control ${not empty ageError ? 'is-invalid' : ''}"
                                 id="age" name="age"
                                 value="${employeeDTO.age}">
                          <span class="text-danger" id="ageError">
                            <c:if test="${not empty ageError}">${ageError}</c:if>
                          </span>
                        </div>

                        <!-- ADDRESS -->
                        <div class="mb-3">
                          <label class="form-label">Address</label>
                          <textarea class="form-control ${not empty addressError ? 'is-invalid' : ''}"
                                    id="address" name="address" rows="2">${employeeDTO.address}</textarea>
                          <span class="text-danger" id="addressError">
                            <c:if test="${not empty addressError}">${addressError}</c:if>
                          </span>
                        </div>

                        <!-- MOBILE (display only) -->
                        <div class="mb-3">
                          <label class="form-label">Mobile Number</label>
                          <input type="text" class="form-control email-locked"
                                 value="${employeeDTO.mobileNumber}" readonly>
                        </div>

                        <!-- GENDER (display only) -->
                        <div class="mb-3">
                          <label class="form-label">Gender</label><br>
                          <input type="radio" name="genderDisplay" value="Male"   disabled
                                 ${employeeDTO.gender == 'Male'   ? 'checked' : ''}> Male &nbsp;
                          <input type="radio" name="genderDisplay" value="Female" disabled
                                 ${employeeDTO.gender == 'Female' ? 'checked' : ''}> Female &nbsp;
                          <input type="radio" name="genderDisplay" value="Other"  disabled
                                 ${employeeDTO.gender == 'Other'  ? 'checked' : ''}> Other
                        </div>

                        <!-- Submit -->
                        <div class="d-grid gap-2 mt-4">
                          <button type="submit" class="btn-update">Update Profile</button>
                        </div>

                      </form>
                    </div>
                  </div>
                </div>
              </div>
            </div>

<script>
function toggleDropdown() {
  document.getElementById('profileDropdown').classList.toggle('show');
}
document.addEventListener('click', function(e) {
  var btn = document.getElementById('profileBtn');
  var dd  = document.getElementById('profileDropdown');
  if (!btn.contains(e.target) && !dd.contains(e.target)) {
    dd.classList.remove('show');
  }
});

// ── JS Validation (only runs when JS is ON) ──
function validateEditForm() {
  let isValid = true;
  clearErrors();

  let name = document.getElementById("userName");
  let age  = document.getElementById("age");
  let addr = document.getElementById("address");

  if (!name.value.trim()) {
    showError(name, "nameError", "Name cannot be empty");
    isValid = false;
  } else if (!/^[A-Za-z ]+$/.test(name.value.trim())) {
    showError(name, "nameError", "Name should contain only letters");
    isValid = false;
  }

  if (age.value === "" || parseInt(age.value) <= 18) {
    showError(age, "ageError", "Age must be greater than 18");
    isValid = false;
  }

  if (!addr.value.trim()) {
    showError(addr, "addressError", "Address cannot be empty");
    isValid = false;
  } else if (addr.value.trim().length < 5) {
    showError(addr, "addressError", "Address must be at least 5 characters");
    isValid = false;
  }

  return isValid;
}

function showError(input, errorId, message) {
  input.classList.add("is-invalid");
  document.getElementById(errorId).innerText = message;
}
function clearErrors() {
  document.querySelectorAll(".form-control").forEach(e => e.classList.remove("is-invalid"));
  ["nameError","ageError","addressError"].forEach(id => {
    var el = document.getElementById(id);
    if (el) el.innerText = "";
  });
}

function previewNewImage(event) {
    var file = event.target.files[0];
    if (file) {
        document.getElementById("photoPreview").src
            = URL.createObjectURL(file);
        document.getElementById("photoName").textContent
            = file.name;
    }
}



// Live validation
document.getElementById("userName").addEventListener("keyup", function() {
  if (!this.value.trim()) {
    showError(this, "nameError", "Name cannot be empty");
  } else if (!/^[A-Za-z ]+$/.test(this.value.trim())) {
    showError(this, "nameError", "Name should contain only letters");
  } else {
    this.classList.remove("is-invalid");
    document.getElementById("nameError").innerText = "";
  }
});
document.getElementById("age").addEventListener("keyup", function() {
  if (this.value === "" || parseInt(this.value) <= 18) {
    showError(this, "ageError", "Age must be greater than 18");
  } else {
    this.classList.remove("is-invalid");
    document.getElementById("ageError").innerText = "";
  }
});
document.getElementById("address").addEventListener("keyup", function() {
  if (!this.value.trim()) {
    showError(this, "addressError", "Address cannot be empty");
  } else if (this.value.trim().length < 5) {
    showError(this, "addressError", "Address must be at least 5 characters");
  } else {
    this.classList.remove("is-invalid");
    document.getElementById("addressError").innerText = "";
  }
});
</script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>