...
.alert-inline.error  .close-btn { color: #842029; }
.alert-inline.success .close-btn { color: #0f5132; }
...
<div class="login-card">
  ...
  <p class="welcome-sub">Sign in to your account</p>
  ...
  <div class="form-area">
        ...
        <c:if test="${not empty success}">
          <div class="alert-inline success" id="loginSuccessBox">
            <span>&#10003;</span>
            <span>${success}</span>
            <button class="close-btn"
                    onclick="document.getElementById('loginSuccessBox').style.display='none'">
              &times;
            </button>
          </div>
        </c:if>
     ...
     <form action="userlogincheck" method="post"
          ...
          <div class="mb-3">
             <input type="email"
                    id="emailInput"
                    name="emailId"
                    class="form-control ${not empty emailIdError ? 'is-invalid' : ''}"
  ...
  <c:if test="${not empty loginError}">
    <div class="alert-inline error" style="margin-top:8px;">
      ${loginError}
    </div>
  </c:if>
...
<script>
/* ── JS ON: client-side validation before form submits ── */
function validateLoginForm() {
    ...
    if (!email.value.trim()) {
        ...
        showError(email, emailErr, 'Please enter a valid @gmail.com email');
        isValid = false;
    }

    // Password empty check
    if (!password.value.trim()) {
        showError(password, passErr, 'Password cannot be empty');
        isValid = false;
...
document.getElementById('emailInput').addEventListener('keyup', function() {
    ...
    if (!this.value.trim()) {
        showError(this, err, 'Email cannot be empty');
    } else if (!pattern.test(this.value.trim())) {
        showError(this, err, 'Please enter a valid @gmail.com email');
    } else {
...
document.getElementById('passwordInput').addEventListener('keyup', function() {
    ...
    if (!this.value.trim()) {
        showError(this, err, 'Password cannot be empty');
    } else if (this.value.length < 8) {
        showError(this, err, 'Password must be at least 8 characters');
    } else {
        this.classList.remove('is-invalid');
        err.innerText = '';
    }
