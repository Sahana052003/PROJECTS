<%@ page isELIgnored="false" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Profile - ${loggedInUser.userName}</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
<link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
* { margin:0; padding:0; box-sizing:border-box; }
body { font-family:'DM Sans',sans-serif; background:#f4f5f7; min-height:100vh; }

.navbar-container {
    height:100px; display:flex;
    justify-content:space-between; align-items:center; width:100%;
}
.navbar-brand { margin-left:25px; }
.nav-right { display:flex; margin-right:25px; align-items:center; position:relative; }
.bg-gradient-purple { background:linear-gradient(90deg,#9370DB,#4B0082); }

.avatar-btn {
    width:48px; height:48px; border-radius:50%;
    background:linear-gradient(135deg,#B19CD9,#7A1FC0);
    color:white; font-weight:bold; font-size:1.4rem;
    display:flex; align-items:center; justify-content:center;
    cursor:pointer; border:none; outline:none;
    overflow:hidden;
    border:2px solid rgba(255,255,255,0.6);
}
.avatar-btn img {
    width:100%; height:100%; object-fit:cover; border-radius:50%;
}

.dropdown-wrapper { position:relative; list-style:none; }
.dropdown-wrapper summary { list-style:none; }
.dropdown-wrapper summary::-webkit-details-marker { display:none; }

.profile-dropdown {
    display:none; position:absolute; top:62px; right:0;
    background:white; border-radius:14px; min-width:250px;
    box-shadow:0 8px 28px rgba(0,0,0,0.18); z-index:999;
    overflow:hidden; border:1px solid #e0daf7;
}
.dropdown-wrapper[open] .profile-dropdown { display:block; }

.dropdown-header {
    padding:16px 18px; display:flex; align-items:center; gap:12px;
    border-bottom:1px solid #f0ecfa;
}
.dropdown-avatar {
    width:42px; height:42px; border-radius:50%;
    background:linear-gradient(135deg,#B19CD9,#7A1FC0);
    color:white; font-weight:bold; font-size:1.1rem;
    display:flex; align-items:center; justify-content:center;
    flex-shrink:0; overflow:hidden;
}
.dropdown-avatar img { width:100%; height:100%; object-fit:cover; }
.dd-name  { font-weight:700; font-size:0.95rem; color:#1a1a2e; }
.dd-email { font-size:0.8rem; color:#888; word-break:break-all; }
.dropdown-item-custom {
    display:flex; align-items:center; gap:12px;
    padding:13px 18px; color:#333; text-decoration:none;
    font-size:0.93rem; border-bottom:1px solid #f5f5f5; transition:background 0.15s;
}
.dropdown-item-custom:hover { background:#f5f0ff; color:#5D0E99; }
.dropdown-item-logout {
    display:flex; align-items:center; gap:12px;
    padding:13px 18px; color:#842029;
    text-decoration:none; font-size:0.93rem; transition:background 0.15s;
}
.dropdown-item-logout:hover { background:#fff5f5; }

/* ── Page ── */
.page { max-width:680px; margin:0 auto; padding:36px 20px 60px; }

/* ── Name card — now contains photo + name ── */
.name-card {
    background:#fff; border-radius:16px; border:1px solid #e5e7eb;
    padding:24px 28px; margin-bottom:16px;
    box-shadow:0 1px 6px rgba(0,0,0,0.05);
    display:flex; align-items:center; gap:20px;
}

/* ★ Profile photo ★ */
.profile-photo {
    width:80px; height:80px; border-radius:50%;
    object-fit:cover;
    border:3px solid #9370DB;
    flex-shrink:0;
    box-shadow:0 2px 10px rgba(147,112,219,0.35);
}
/* Fallback letter circle when no image */
.profile-photo-placeholder {
    width:80px; height:80px; border-radius:50%;
    background:linear-gradient(135deg,#B19CD9,#7A1FC0);
    display:flex; align-items:center; justify-content:center;
    color:white; font-weight:700; font-size:2rem;
    flex-shrink:0; border:3px solid #9370DB;
}

.profile-name  { font-size:1.4rem; font-weight:700; color:#111827; margin:0; }
.profile-email { font-size:0.85rem; color:#888; margin:4px 0 0; }

/* Bio card */
.bio-card {
    background:#fff; border-radius:16px; border:1px solid #e5e7eb;
    padding:24px 28px; box-shadow:0 1px 6px rgba(0,0,0,0.05);
}
.bio-card-title {
    font-size:1rem; font-weight:700; color:#111827;
    margin-bottom:16px; padding-bottom:12px;
    border-bottom:1px solid #f3f4f6;
}
.bio-row {
    display:flex; justify-content:space-between; align-items:center;
    padding:12px 0; border-bottom:1px solid #f9fafb;
}
.bio-row:last-child { border-bottom:none; padding-bottom:0; }
.bio-label { font-size:0.85rem; color:#9ca3af; font-weight:500; }
.bio-value { font-size:0.92rem; font-weight:600; color:#111827; }

/* Edit button */
.btn-edit {
    display:inline-block; margin-top:20px;
    background:linear-gradient(45deg,#B19CD9,#7A1FC0);
    color:white; font-weight:700; font-size:0.95rem;
    padding:10px 28px; border-radius:50px; text-decoration:none;
    box-shadow:0 4px 10px rgba(0,0,0,0.2); transition:all 0.3s ease;
}
.btn-edit:hover {
    background:linear-gradient(45deg,#A083D1,#5D0E99);
    color:white; transform:scale(1.03);
}
</style>
</head>
<body>

<!-- ── Navbar ── -->
<nav class="navbar bg-gradient-purple">
  <div class="container-fluid navbar-container">
    <a class="navbar-brand" href="#">
      <img src="data:image/jpeg;base64,"
         alt="X-Workz Logo" height="70"
         style="border-radius:50%; object-fit:cover;">
    </a>

    <div class="nav-right">
      <details class="dropdown-wrapper">

        <%-- Navbar avatar: show photo or letter --%>
        <summary class="avatar-btn">
          <c:choose>
            <c:when test="${not empty loggedInUser.imagePath}">
              <c:url var="navImg" value="/serveImage">
                <c:param name="path" value="${loggedInUser.imagePath}"/>
              </c:url>
              <img src="${navImg}"
                   alt="${fn:toUpperCase(fn:substring(loggedInUser.emailId,0,1))}"
                   onerror="this.style.display='none';
                            this.nextElementSibling.style.display='flex';">
              <span style="display:none;">${fn:toUpperCase(fn:substring(loggedInUser.emailId,0,1))}</span>
            </c:when>
            <c:otherwise>
              ${fn:toUpperCase(fn:substring(loggedInUser.emailId, 0, 1))}
            </c:otherwise>
          </c:choose>
        </summary>

        <div class="profile-dropdown">
          <div class="dropdown-header">

            <%-- Dropdown avatar --%>
            <div class="dropdown-avatar">
              <c:choose>
                <c:when test="${not empty loggedInUser.imagePath}">
                  <c:url var="ddImg" value="/serveImage">
                    <c:param name="path" value="${loggedInUser.imagePath}"/>
                  </c:url>
                  <img src="${ddImg}"
                       alt="${fn:toUpperCase(fn:substring(loggedInUser.emailId,0,1))}"
                       onerror="this.style.display='none';
                                this.parentElement.innerText=
                                '${fn:toUpperCase(fn:substring(loggedInUser.emailId,0,1))}';">
                </c:when>
                <c:otherwise>
                  ${fn:toUpperCase(fn:substring(loggedInUser.emailId,0,1))}
                </c:otherwise>
              </c:choose>
            </div>

            <div>
              <div class="dd-name">${loggedInUser.userName}</div>
              <div class="dd-email">${loggedInUser.emailId}</div>
            </div>
          </div>

          <a href="profilepage?id=${loggedInUser.id}" class="dropdown-item-custom">View Profile</a>
          <a href="editProfile?id=${loggedInUser.id}" class="dropdown-item-custom">Edit Profile</a>
          <a href="myaccount?id=${loggedInUser.id}" class="dropdown-item-custom">My Account</a>
          <a href="login" class="dropdown-item-logout">Logout</a>
        </div>

      </details>
    </div>
  </div>
</nav>

<!--Page-->
<div class="page">

  <!-- Box 1: Photo,Name -->
  <div class="name-card">

    <%--  Profile photo with serveImage endpoint  --%>
    <c:choose>
      <c:when test="${not empty loggedInUser.imagePath}">
        <c:url var="profileImg" value="/serveImage">
          <c:param name="path" value="${loggedInUser.imagePath}"/>
        </c:url>
        <img class="profile-photo"
             src="${profileImg}"
             alt="Profile Photo"
             onerror="this.style.display='none';
                      this.nextElementSibling.style.display='flex';">
        <div class="profile-photo-placeholder" style="display:none;">
          ${fn:toUpperCase(fn:substring(loggedInUser.userName, 0, 1))}
        </div>
      </c:when>
      <c:otherwise>
        <div class="profile-photo-placeholder">
          ${fn:toUpperCase(fn:substring(loggedInUser.userName, 0, 1))}
        </div>
      </c:otherwise>
    </c:choose>

    <div>
      <p class="profile-name">
        <c:choose>
          <c:when test="${not empty loggedInUser.userName}">${loggedInUser.userName}</c:when>
          <c:otherwise>Unknown User</c:otherwise>
        </c:choose>
      </p>
      <p class="profile-email">${loggedInUser.emailId}</p>
    </div>
  </div>

  <!-- Box 2: Bio -->
  <div class="bio-card">
    <div class="bio-card-title">Bio</div>

    <div class="bio-row">
      <span class="bio-label">Name</span>
      <span class="bio-value">
        <c:choose>
          <c:when test="${not empty loggedInUser.userName}">${loggedInUser.userName}</c:when>
          <c:otherwise>—</c:otherwise>
        </c:choose>
      </span>
    </div>

    <div class="bio-row">
      <span class="bio-label">Age</span>
      <span class="bio-value">
        <c:choose>
          <c:when test="${not empty loggedInUser.age}">${loggedInUser.age}</c:when>
          <c:otherwise>—</c:otherwise>
        </c:choose>
      </span>
    </div>

    <div class="bio-row">
      <span class="bio-label">Address</span>
      <span class="bio-value">
        <c:choose>
          <c:when test="${not empty loggedInUser.address}">${loggedInUser.address}</c:when>
          <c:otherwise>—</c:otherwise>
        </c:choose>
      </span>
    </div>

  </div>

</div>

<script>
document.addEventListener('click', function(e) {
    const wrapper = document.querySelector('.dropdown-wrapper');
    if (wrapper && !wrapper.contains(e.target)) {
        wrapper.removeAttribute('open');
    }
});
</script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>